...
<script>
  import {
    query_regex_extractor,delete_regex_extractor
  } from "./api_result";
  ...
  export default {
    ...
    methods: {
      ...
      fresh(page_index=1){
        ...
        let $this = this;
        query_regex_extractor({"list_query":query}).then(function (res) {
      ...
      // 删除
      handle_delete(row) {
        this.$confirm('此操作将永久删除该正则化抽取器管理信息, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          let $this = this;
          delete_regex_extractor(row.regex_extractor_id).then(function(){
            $this.$message({
              type: 'success',
              message: '删除成功!'
            });
