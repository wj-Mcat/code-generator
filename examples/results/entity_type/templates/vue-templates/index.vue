...
<script>
  import {
    query_entity_type,delete_entity_type
  } from "./api_result";
  ...
  export default {
    ...
    methods: {
      ...
      fresh(page_index=1){
        ...
        let $this = this;
        query_entity_type({"list_query":query}).then(function (res) {
      ...
      // 删除
      handle_delete(row) {
        this.$confirm('此操作将永久删除该槽位类别管理信息, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          let $this = this;
          delete_entity_type(row.entity_type_id).then(function(){
            $this.$message({
              type: 'success',
              message: '删除成功!'
            });
