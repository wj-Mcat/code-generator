  ...
  export default {
    ...
    methods: {
      ...
      fresh(page_index=1){
        ...
        let $this = this;
        query_entity({"list_query":query}).then(function (res) {
      ...
      // 删除
      handle_delete(row) {
        this.$confirm('此操作将永久删除该槽位数据管理信息, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          let $this = this;
          delete_entity(row.entity_id).then(function(){
            $this.$message({
              type: 'success',
              message: '删除成功!'
            });
